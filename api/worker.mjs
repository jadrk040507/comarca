...
export function validate(input){
 ...
 const name=text('name',120,true),email=text('email',254,true),activity=text('activity',120,true),message=text('message',1800),phone=text('phone',30),token=text('token',2048,true);
 if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)||!activities.includes(activity)||phone&&!/^[+\d\s()-]{7,30}$/.test(phone))throw Error('Revisa el correo, la actividad y el teléfono.');
 ...
 return {name,email,activity,message,phone,token,emailOptIn:input.emailOptIn===true,whatsappOptIn:input.whatsappOptIn===true};
}
...
export async function handle(request,env,fetcher=fetch){
 ...
 const saved=await fetcher('https://api.notion.com/v1/pages',{method:'POST',headers:{Authorization:`Bearer ${env.NOTION_TOKEN}`,'Notion-Version':'2025-09-03','Content-Type':'application/json'},body:JSON.stringify(notionPayload(v,env.NOTION_DATA_SOURCE_ID)),signal:AbortSignal.timeout(15000)});
