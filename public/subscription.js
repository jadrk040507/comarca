const endpoint='https://la-comarca-formularios.la-comarca.workers.dev/calendario.ics';
export function subscriptionURL({types=[],circles=[],allTypes=true,allCircles=true}){
...
export function initSubscription(events){
 ...
 form.addEventListener('submit',event=>{event.preventDefault();const f=new FormData(form),allTypes=f.has('allTypes'),types=f.getAll('type');if(!allTypes&&!types.length){status.textContent='Elige al menos un tipo de actividad.';return;}const url=subscriptionURL({types,allTypes,circles:f.getAll('circle'),allCircles:f.has('allCircles')});urlField.value=url;document.querySelector('#subscription-apple').href=url.replace(/^https:/,'webcal:');output.hidden=false;status.textContent='Enlace listo. Añádelo como suscripción, no como archivo importado.';});
