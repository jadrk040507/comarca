...
create policy profiles_self on public.profiles for select using (id = (select auth.uid()));
create policy workspaces_member on public.workspaces for select using (public.is_member(id));
create policy memberships_self on public.workspace_memberships for select using (user_id = (select auth.uid()));
create policy activities_member on public.activities for select using (status = 'published' or public.is_member(workspace_id));
create policy activities_editor_insert on public.activities for insert with check (public.is_member(workspace_id, array['admin','coordinator','editor']::public.member_role[]));
create policy activities_editor_update on public.activities for update using (public.is_member(workspace_id, array['admin','coordinator','editor']::public.member_role[])) with check (public.is_member(workspace_id, array['admin','coordinator','editor']::public.member_role[]));
create policy activities_editor_delete on public.activities for delete using (public.is_member(workspace_id, array['admin','coordinator','editor']::public.member_role[]));
create policy materials_member on public.materials for select using (status = 'published' or public.is_member(workspace_id));
create policy materials_editor_insert on public.materials for insert with check (public.is_member(workspace_id, array['admin','coordinator','editor']::public.member_role[]));
create policy materials_editor_update on public.materials for update using (public.is_member(workspace_id, array['admin','coordinator','editor']::public.member_role[])) with check (public.is_member(workspace_id, array['admin','coordinator','editor']::public.member_role[]));
create policy materials_editor_delete on public.materials for delete using (public.is_member(workspace_id, array['admin','coordinator','editor']::public.member_role[]));
